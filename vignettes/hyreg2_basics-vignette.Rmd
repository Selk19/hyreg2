...
editor_options: 
  markdown: 
    wrap: sentence
vignette: >
  %\VignetteIndexEntry{hyreg2_basics}
...
However, it can be applied to any situation in which a combined likelihood of continuous and dichotomous data should be maximized.
(XXX BIOSTATISTICS)
...
## Estimating models using variables-and-parameters-formula

In the `hyreg2` function, linear and non-linear formulas can be provided including variables and parameters.
For this purpose, `formula_type_classic` must be set to `FALSE`.
In the given formula, additional parameters must be provided which are no column names in the datset e.g.`y ~ 1/exp(x1 * beta1 + x2 * beta2)` or `y ~ (x1 * beta1 + x2 * beta2 ) * (x1* beta1 + x3 * beta3)`where`beta` are the parameters to be estimated.
We simulate a new column called `y_non` for the dataset simulated_data_norm and show, how to estimate a model using a variables-and-parameters-formula.
...
If an intercept should be estimated, this must be explicitly given in for formula and named `"INTERCEPT"`.
The provided input for `stv` must now contain the additional parameter names and not the variable names from the dataset.
...
If partial coefficients should be used, the provided variabales_vectors must contain the names of the additional parameters and not the variable names from the dataset.
